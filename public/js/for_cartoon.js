...
const cartoon_id = new URLSearchParams(window.location.search).get("id")
let is_favorite = false

async function check_favorite(){
    try{
        const res = await fetch(`/api/fav_cartoons/${cartoon_id}/check`,{
            credentials:"include"
        })
...
async function toggle_favorite() {
    try{
        if(is_favorite){
            await fetch(`/api/fav_cartoons/${cartoon_id}`,{
                method:"DELETE",
                credentials:"include"
            })
            is_favorite = false
            ...
            await fetch(`/api/fav_cartoons`,{
                method:"POST",
                credentials:"include",
                headers:{"Content-Type":"application/json"},
                body:JSON.stringify({cartoon_id})
            })
            is_favorite = true
        }
        update_btn()
...
async function show(){
    try{
        ...
        const res = await fetch(`/api/cartoons/${cartoon_id}`, {credentials: "include"})
